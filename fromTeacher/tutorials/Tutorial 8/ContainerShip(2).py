...
class Ship:
    def __init__(self, length, width):
        self.piles = [[ContainerPile() for _ in range(width)] for _ in range(length)]

    def getPiles(self):
        return self.piles
    
    def loadContainer(self, container):
        pile = self.getLighestPile()
        pile.pileContainer(container)

    def getLighestPile(self):
        lighestWeight = 100000000000
        lightestPile = None
        for row in self.piles:
            for pile in row:
                if pile.getTotalWeight()<lighestWeight:
                    lightestPile = pile
                    lighestWeight = pile.getTotalWeight()
        return lightestPile
...
class Printer:
    ...
    def printShip(self, ship, outputFile):
        for row in ship.getPiles():
            for pile in row:
                self.printContainerPile(pile, outputFile)
                outputFile.write("--- total weight {0:d}\n".format(pile.getTotalWeight()))
...
containers = tester.createRandomContainerSample(100)
for container in containers:
    ship.loadContainer(container)
 
printer.printShip(ship, sys.stdout)
